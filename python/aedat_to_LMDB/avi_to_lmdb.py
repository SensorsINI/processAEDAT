    ...
    for dirpath, dirnames, filenames in os.walk(dir_to_walk):
        for filename in (f for f in filenames if f.endswith(".avi")):
            ...
            if math.isinf(vid._meta['nframes']):
                print("The following avi movie has too many frames: ", label_filenames[-1])
                sys.exit()
...
def avi_to_frame_list(avi_filename, gray):
    ...
    try:
        ...
        print "Could not load meta information for file %".format(avi_filename)
        return None
    if gray:
...
def write_categ_lmdb(workfile, categories, LMDB_path, gray):
    ...
    for idx, categ in enumerate(categories):
        categ_db.append(lmdb.open(os.path.join(LMDB_path, categ), map_size=int(1e12),
...
def write_train_test_lmdb(categories, LMDB_path, train_test_split, num_rot):
    ...
    indices_train_database = np.arange(len(categories)*train_idx*(num_rot+1))
    print("total training samples ", len(indices_train_database))
    np.random.shuffle(indices_train_database)
    indices_test_database = np.arange(len(categories)*test_idx*(num_rot+1))
    ...
    for idx, categ in enumerate(categories):
        ...
        with categ_db.begin() as categ_txn:
            ...
            # shuffle the images corresponding to each category
            ...
            with train_db.begin(write=True) as in_txn_train:
                for i in range(train_idx):
                    if i%100000 == 0:
                        ...
                        last = time.time()
                        print("Wrote {} training samples to database in {} seconds".format(i*(num_rot+1),
                              elapsed))
                    in_idx = indices_categ[i]
                    in_key = DB_KEY_FORMAT.format(in_idx)
                    in_item = categ_txn.get(in_key)

                    out_key = DB_KEY_FORMAT.format(indices_train_database[i*(num_rot+1) + curr_train_idx])
                    in_txn_train.put(out_key, in_item)

                    # Rotate images by 90 degrees
                    datum = caffe.proto.caffe_pb2.Datum()
                    datum.ParseFromString(in_item)
                    assert datum.channels == 1, "The algorithm currently only works for 1-channel images"
                    flat_x = np.fromstring(datum.data, dtype=np.uint8)
                    x = flat_x.reshape(datum.height, datum.width)
                    y = datum.label
                    for rotation_idx in range(num_rot):
                        x = np.rot90(x)
                        out_image = np.reshape(x, (datum.channels, datum.height, datum.width))
                        im_dat = caffe.io.array_to_datum(out_image.astype('uint8'),
                                                     label=int(y))
                        out_key = DB_KEY_FORMAT.format(indices_train_database[i*(num_rot+1) + curr_train_idx
                                                                              + rotation_idx + 1])
            ...
            with test_db.begin(write=True) as in_txn_test:
                for i in range(test_idx):
                    if i%100000 == 0:
                        print("Wrote {} testing samples to database".format(i*(num_rot+1)))
                    in_idx = indices_categ[train_idx + i]
                    in_key = DB_KEY_FORMAT.format(in_idx)
                    in_item = categ_txn.get(in_key)

                    out_key = DB_KEY_FORMAT.format(indices_test_database[i*(num_rot+1) + curr_test_idx])
...
if __name__ == '__main__':
    ...
    parser.add_argument('--workfile_train', default='workfile_train.txt',
    ...
    parser.add_argument('--LMDB_train_path', default='./lmdb_train/',
                        help='Where to write out the LMDB dataset.')
    parser.add_argument('--avi_dir_train', default='/media/iulialexandra/Storage/inilabs-nas/RoShamBoNPP/recordings',
                        help='Where to look for .avi files to add to the\
                        training LMDB')
    parser.add_argument('--working_dir', default='.',
                        help='Where to create the workfile used for \
                        creating the LMDB')
    parser.add_argument('--labels_dir_train', default='labels_train',
                        help='Where to create the labels files for training')
    ...
    parser.add_argument('--num_rotations', default=0, type=int,
                        help='How many 90 degree rotations to perform for each image')
    parser.add_argument('--gray', default=True,
                        help='If the input data is grayscale, the output' +
                        ' LMDB images will have only one channel. Otherwise' +
    ...
    if not os.path.exists(args.LMDB_train_path):
        os.makedirs(args.LMDB_train_path)

    # Make the workfiles used to create the LMDBs
    create_workfile(args.avi_dir_train, args.working_dir, args.workfile_train,
                    args.labels_dir_train)


    # Create the train and test LMDBs
    start_time = time.time()
    curr_idx = write_categ_lmdb(args.workfile_train, args.categories,
                                args.LMDB_train_path, args.gray)
    print("Number of samples for each category:{}".format(curr_idx))

    write_train_test_lmdb(args.categories, args.LMDB_train_path,
