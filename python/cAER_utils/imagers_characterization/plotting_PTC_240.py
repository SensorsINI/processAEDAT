...
####################################################
PTC_data_file = 'Z:/Characterizations/Measurements_final/DAVIS240C/PTC/DAVIS240C_ADCext_ptc_1000lux_24_08_16-14_14_40/saved_variables/variables_DAVIS240C.npz'
...
if (plot_signal_exposure):
    ...
    plt.title('240C Mean Signal vs Exposure')
    ...
    for this_area_x in range(len(frame_x_divisions)):
        for this_area_y in range(len(frame_y_divisions)):
            ...
            ax1.text(50, 200, 'Linearity error: ' + str(format(linearity_error, '.2f') + '%'), color='r')
            ax2 = ax1.twinx()
    ...
    for tl in ax2.get_yticklabels():
        tl.set_color('r')
    plt.savefig(output_dir + "APS_signal_exposure_240C.pdf", format='pdf', bbox_inches='tight')
    plt.savefig(output_dir + "APS_signal_exposure_240C.png", format='png', bbox_inches='tight', dpi=600)
...
if (plot_PTC_linear_fit):
    ...
    plt.figure()
    plt.title('240C Temporal Variance vs Signal')
    percentage_ignore = 0.2
    uV_DN = (1.501 / 1024.0) * 1000000.0
    for this_area_x in range(len(frame_x_divisions)):
        for this_area_y in range(len(frame_y_divisions)):
            ...
            plt.plot(spatiotemporal_mean.reshape(len(spatiotemporal_mean)),
                     ...
                     label='Fitted')
            plt.text(300, 2, 'Slope: ' + str(format(slope, '.3f')), color='k')
            plt.text(300, 1, 'Conversion gain: ' + str(format(uV_DN * slope, '.3f')) + 'uV/e', color='k')
            plt.annotate(
    ...
    plt.legend(loc=2, frameon=True)
    plt.savefig(output_dir + "PTC_linear_fit_240C.pdf", format='pdf', bbox_inches='tight')
    plt.savefig(output_dir + "PTC_linear_fit_240C.png", format='png', bbox_inches='tight', dpi=600)
...
if (plot_FPN):
    ...
    plt.figure()
    plt.title('240C Row/Column FPN vs Signal')
    for this_area_x in range(len(frame_x_divisions)):
        for this_area_y in range(len(frame_y_divisions)):
            ...
            ind_50 = np.where(spatiotemporal_mean[:, this_area_y, this_area_x] >= 220.0)[0][0]
    ...
    plt.ylabel('FPN [DN]')
    plt.legend(loc=2, frameon=True)
    plt.savefig(output_dir + "APS_FPN_240C.pdf", format='pdf', bbox_inches='tight')
    plt.savefig(output_dir + "APS_FPN_240C.png", format='png', bbox_inches='tight', dpi=600)
...
if (plot_SNR):
    ...
    for this_area_x in range(len(frame_x_divisions)):
        for this_area_y in range(len(frame_y_divisions)):
            ...
            ind_100 = np.where(spatiotemporal_mean[:, this_area_y, this_area_x] >= 425.0)[0][0]
            plt.annotate(
    ...
    plt.ylabel('SNR [DB]')
    plt.ylim(0, 50)
    plt.savefig(output_dir + "APS_SNR_240C.pdf", format='pdf', bbox_inches='tight')
    plt.savefig(output_dir + "APS_SNR_240C.png", format='png', bbox_inches='tight', dpi=600)
