function toy_layer_model_example
...
tempTable(1,2,:) = lambdaFB;
tempTable(2,1,:) = lambdaD;

%z11-l11-z21
psi{3}.Member = [0, 2, 4];
tmp = tempTable;
tmp(1, 1, 1) = abs(im1(1)-im2(1));
tmp(1, 2, 1) = lambdaFB;            % FG occluded by BG
psi{3}.P = exp(-tmp);
%z11-l11-z22
psi{4}.Member = [0, 3, 4];
tmp = tempTable;
tmp(1, 1, 2) = abs(im1(1)-im2(2));
tmp(1, 2, 2) = lambdaFB;            % FG occluded by BG
psi{4}.P = exp(-tmp);
...
% Backward frame 2->1
tempTable     = zeros(2,2,2);
tempTable(2,1,:) = lambdaFB;
tempTable(1,2,:) = lambdaD;

%z21-l21-z11
psi{7}.Member = [0, 2, 6];
tmp = tempTable;
tmp(1, 1, 1) = abs(im2(1)-im1(1));
tmp(2, 1, 1) = lambdaFB;            % FG occluded by BG
psi{7}.P = exp(-tmp);
%z21-l21-z22
t=8;
psi{t}.Member = [1, 2, 6];
tmp = tempTable;
tmp(1, 1, 2) = lambdaD;
tmp(2, 1, 2) = lambdaFB;            % FG occluded by BG
psi{t}.P = exp(-tmp);
