...
//    outMatrix = mxGetPr(plhs[0]);
    ...
    plhs[0] = mxCreateSparse(nPixels, nPixels, nzmax, mxREAL);
    sr  = mxGetPr(plhs[0]);	// pointer to real values
    irs = mxGetIr(plhs[0]); // pointer to row index
    jcs = mxGetJc(plhs[0]); // pointer to column index
