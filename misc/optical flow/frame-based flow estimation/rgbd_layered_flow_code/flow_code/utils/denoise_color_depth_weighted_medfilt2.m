function uvo = denoise_color_depth_weighted_medfilt2(uv, im, occ, bfhsz, mfsz, sigma_i, depth, sigma_d, fullVersion)
%%
% edge region: weighted median filtering, the weights are determined by
%  spatial distance, intensity distance, occlusion state
% smooth region: 

occ(isnan(depth)) = occ(isnan(depth))/100; % disable occluded in depth map
depth(isnan(depth)) = nanmean(depth(:));
...
pad_depth = padarray(depth, bfhsz*[1 1], 'symmetric', 'both');        
...
for ib = 1:nB;
    ...
    % depth weight
    tmp = (pad_depth(pad_indx) - repmat(pad_depth(cindx(:))', [(bfhsz*2+1)^2, 1])).^2;
    tmp = exp(-tmp/2/sigma_d^2);
    tmp(isnan(tmp))=nanmean(tmp(:)); % one of the two pixels is occluded
    weights = weights.*tmp;   
