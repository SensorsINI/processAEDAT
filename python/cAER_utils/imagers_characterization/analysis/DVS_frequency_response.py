...
#                        
#                        for Z:/Characterizations/Measurements/DAVIS208_frequency_response_15_08_16-10_29_42/ onwards
#                        yaddr = yaddr[yaddr>50]
#                        pol = pol[yaddr>50]
#                        ts = ts[yaddr>50]
#                        xaddr = xaddr[yaddr>50]
##                        
#                        yaddr = yaddr[xaddr>45]
#                        pol = pol[xaddr>45]
#                        ts = ts[xaddr>45]
#                        xaddr = xaddr[xaddr>45]
#                        for nd2
...
#                        xaddr = xaddr[xaddr>50]
                        #last nd2of sdavis
#                        yaddr = yaddr[xaddr<100]
#                        pol = pol[xaddr<100]
#                        ts = ts[xaddr<100]
#                        xaddr = xaddr[xaddr<100]
#                        yaddr = yaddr[yaddr<100]
#                        pol = pol[yaddr<100]
#                        ts = ts[yaddr<100]
#                        xaddr = xaddr[xaddr<100]
#                        yaddr = yaddr[yaddr>50]
...
#            print "Selected pixel [" + str(ind_x_max) + "," + str(ind_y_max) + "]"
            ...
            for this_ev in range(len(ts)):
                if (ts[this_ev] >= (sync_ts[-1] + 1000000) and ts[this_ev] < (sync_ts[-1] + 2000000)): # noise events in the nxt 1 out of 3 sec
                        if (pol[this_ev] == 1):
                ...
                if(ts[this_ev]<=sync_ts[-1]):
                    if((ts[this_ev] >= (sync_ts[this_sync_ts] + 4.0*sine_phase)) and (this_sync_ts<=len(sync_ts)-1)):                           
                        ...
                        if(this_sync_ts%5000==0 or this_sync_ts<=10):
