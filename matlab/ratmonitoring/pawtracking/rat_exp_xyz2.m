...
function a = rat_exp_xyz2(pathfile,smooth_factor,t1,t2,d1,d2)
...
for n=1:clustermax
   ...
   plot3(smooth(exp_data(d1:d2,2),smooth_factor),smooth(exp_data(d1:d2,4),smooth_factor),smooth(exp_data(d1:d2,3),smooth_factor),colorc); 
  ...
  plot3(xdata(d1:d2),ydata(d1:d2),zdata(d1:d2),'b'); 
  
  
  if isempty(i1) | isempty(i2)
  else
  plot3(xdata(i1),ydata(i1),zdata(i1),'bo');
  ...
  exp_data = data;
  plot3(smooth(exp_data(d1:d2,2),smooth_factor),smooth(exp_data(d1:d2,4),smooth_factor),smooth(exp_data(d1:d2,3),smooth_factor),'g'); 
