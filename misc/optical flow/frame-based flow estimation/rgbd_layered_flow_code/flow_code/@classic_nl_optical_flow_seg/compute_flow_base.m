function [uv this]= compute_flow_base(this, uv)
%%
...
% THIS SOFTWARE.        
  ...
  elseif isa(this.rho_spatial_u{1}, 'gsm_density')
      for i = 1:length(this.rho_spatial_u)
          qua_this.rho_spatial_u{i}   = robust_function('quadratic',...
              sqrt(1/this.rho_spatial_u{i}.precision));
          qua_this.rho_spatial_v{i}   = robust_function('quadratic',...
              sqrt(1/this.rho_spatial_v{i}.precision));
      end;
  else
  ...
  elseif isa(qua_this.rho_data, 'gsm_density')
      qua_this.rho_data        = robust_function('quadratic',...
          sqrt(1/this.rho_data.precision));
  else
      error('evaluate_log_posterior: unknown rho function!');
  end;
...
%           this = detect_flow_edges(this, uv);
%           qua_this.flowEdges = this.flowEdges;
%       end
% 
%       if ~isempty(this.occMethod)
%           this = detect_occlusion(this, uv);
%           qua_this.occ = this.occ;
%       end
    ...
    for j = 1:this.max_linear       
        ...
        switch (lower(this.solver))
            ...
            case 'pcg'
                %[x flag] = pcg(A,b, [], this.pcg_iters); 
                [x flag] = pcg(A,b, [], this.pcg_iters, diag(diag(A))); 
                x        = reshape(x, size(uv));
            otherwise
...
%             occ = detect_occlusion(uv, this.images); % bug??
...
%     occ = occ.*(1-this.occ);
%     fprintf('.');
%     figure; imagesc(occ); colormap gray;
% end
...
%             uv = denoise_color_weighted_medfilt2(uv, this.color_images,...
%                 occ, this.area_hsz, this.median_filter_size, this.sigma_i, this.fullVersion);
%             
%         end;        
%         
%         duv = uv - uv0;
