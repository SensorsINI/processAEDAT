...
####################################################
PTC_data_file = 'Z:/Characterizations/Measurements_final/346F/PTC/DAVIS346B_ADCint_ptc_20_06_16-15_37_22_light/saved_variables/variables_DAVIS346B.npz'
...
if (plot_signal_exposure):
    ...
    plt.title('346F Mean Signal vs Exposure')
    ...
    for this_area_x in range(len(frame_x_divisions)):
        for this_area_y in range(len(frame_y_divisions)):
            ...
            for i in range(len(exposures_fit)):
                partial_slope, temp = np.polyfit(exposures[ind_fit_lower - 1:ind_fit_lower + i].reshape(
                    len(exposures[ind_fit_lower - 1:ind_fit_lower + i])),
                                                 spatiotemporal_mean[ind_fit_lower - 1:ind_fit_lower + i].reshape(
                                                     len(spatiotemporal_mean[ind_fit_lower - 1:ind_fit_lower + i])), 1)
    ...
    for tl in ax2.get_yticklabels():
        tl.set_color('r')
    plt.savefig(output_dir + "APS_signal_exposure_346F.pdf", format='pdf', bbox_inches='tight')
    plt.savefig(output_dir + "APS_signal_exposure_346F.png", format='png', bbox_inches='tight', dpi=600)
...
if (plot_PTC_linear_fit):
    ...
    plt.figure()
    plt.title('346F Temporal Variance vs Signal')
    percentage_ignore = 0.2
    ...
    plt.legend(loc=2, frameon=True)
    plt.savefig(output_dir + "PTC_linear_fit_346F.pdf", format='pdf', bbox_inches='tight')
    plt.savefig(output_dir + "PTC_linear_fit_346F.png", format='png', bbox_inches='tight', dpi=600)
...
if (plot_FPN):
    ...
    plt.figure()
    plt.title('346F Row/Column FPN vs Signal')
    for this_area_x in range(len(frame_x_divisions)):
        for this_area_y in range(len(frame_y_divisions)):
    ...
    plt.ylabel('FPN [DN]')
    plt.legend(loc=2, frameon=True)
    plt.savefig(output_dir + "APS_FPN_346F.pdf", format='pdf', bbox_inches='tight')
    plt.savefig(output_dir + "APS_FPN_346F.png", format='png', bbox_inches='tight', dpi=600)
...
if (plot_SNR):
    ...
    plt.ylabel('SNR [DB]')
    plt.ylim(0, 50)
    plt.savefig(output_dir + "APS_SNR_346F.pdf", format='pdf', bbox_inches='tight')
    plt.savefig(output_dir + "APS_SNR_346F.png", format='png', bbox_inches='tight', dpi=600)
