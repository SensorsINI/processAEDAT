...
for trial=1:numOfTrials
    bins2 = [zeros(64,2) bins{trial} zeros(64,2)];
    hold on;
    for ch=1:64
    ...
    line([trial*numOfBins trial*numOfBins],[0 65],'Color','b');
    if ischar((nameOfTrials(trial)))
        text((trial-1)*numOfBins,65,nameOfTrials(trial),'FontSize',12)
    else
        text((trial-1)*numOfBins,65,num2str(nameOfTrials(trial)),'FontSize',12);
    end
    pause(1);
end
...
%     end
%     line([CutAtTime(trial) CutAtTime(trial)],[0 65],'Color','b');
