function flow_operator_test(model)
...
[data.Zt, data.Zx, data.Zy] = partial_deriv(model.depths, uv, ...
    model.interpolation_method, model.deriv_filter, model.blend);
[A, b] = flow_operator(model, uv3, zeros(size(uv3)), data);
Duv3    = b;
...
for i = 1:numel(uv3)
    ...
    L1 = evaluate_log_posterior(model, uv3p);
    L2 = evaluate_log_posterior(model, uv3m);
