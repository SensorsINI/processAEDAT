...
% THIS SOFTWARE.        
  ...
  if size(this.iamges,4) > 1
      grayImages = [];
      labImages = [];
      for i = 1:size(this.iamges,4)
          ...
          labImages = cat(labImage, im1);
      end;
      
  else
      grayImages = double(this.images);
      labImages = double(this.images);
  end;
  ...
  if this.texture      
      ...
      images  = structure_texture_decomposition_rof(grayImages, 1/8, 100, this.alp);        
  else
  ...
  org_pyramid_images = compute_image_pyramid(grayImages, f, this.pyramid_levels, 1/this.pyramid_spacing);
  org_color_pyramid_images = compute_image_pyramid(labImages, f, this.pyramid_levels, 1/this.pyramid_spacing);
  ...
  % For segmentation purpose
  org_gnc_pyramid_images = compute_image_pyramid(grayImages, f, this.gnc_pyramid_levels, 1/this.gnc_pyramid_spacing);   
  org_color_gnc_pyramid_images = compute_image_pyramid(labImages, f, this.gnc_pyramid_levels, 1/this.gnc_pyramid_spacing);   
